...
int Gen_pulse::get_Duty(double _t_p, int _freq) {
  // D = tp/T = tp*freq
  int _Duty = ceil((_t_p * _freq * pow(2, this->resolution)));
  // Serial.print(_Duty);
  _Duty = constrain(_Duty, 0, pow(2, this->resolution));
  // Serial.print(_Duty);
  return _Duty;
};
...
void Gen_pulse::Change_PFM(double discrepancy) {
  if (micros() - prev_time_pwm > 500) {
    ...
    cur_freq = this->freq;
    // D = (Uout + dU) /Uin = Uout/Uin + dU/Uin = refD + D'
    if (discrepancy > deviationPFM || discrepancy < -deviationPFM) {

      this->freq += ceil(discrepancy / (ADC_in_MAX * this->t_p));
      this->freq = constrain(this->freq, 100, MAX_freq);
      if (cur_freq == this->freq) {
        prev_time_pwm = micros();
        return;
      }
      ledcChangeFrequency(this->channel, this->freq, this->resolution);
      int _Duty = get_Duty(this->t_p, this->freq);
      if (_Duty != cur_duty) {
        ledcWrite(this->channel, _Duty);
        this->Duty = _Duty;
      }
...
void Gen_pulse::Change_Hyst(double discrepancy) {
  ...
  // переводим в вольты, в реальные границы по напряжению
  if (discrepancy > this->up_window) {
    // this->Duty= pow(2,this->resolution)*0.5;
    this->Duty = 208;
    // Serial.print("Change_Hyst:");
    // Serial.println("HIGH");
